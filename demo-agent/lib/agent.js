...
import { createChallengeTool, verifyAuthTool } from "../../mcp-server/lib/tools/auth.js";
    ...
    if (response.stop_reason === "tool_use") {
      ...
      for (const block of response.content) {
        if (block.type === "tool_use") {
          ...
          const result = await callTool(block.name, /** @type {Record<string, unknown>} */ (block.input));
