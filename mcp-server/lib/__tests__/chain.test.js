 ...
 */
import {extractDelegatedFrom, verifyDelegationChain} from '../core/chain.js';
import {sha256} from '@noble/hashes/sha2.js';
import {toBase64url} from '../core/crypto.js';

const {generateKeyPair} = await import('../core/crypto.js');
...
async function make2HopChain() {
  ...
  // root → mid
  const rootVc = await issueCredential(
    MID_DID, {delegatedFrom: null}, ROOT_DID, rootKp, 3600
  );

  // mid → leaf (delegatedFrom = sha256 of rootVc.jwt)
  const delegatedFrom = sha256base64url(rootVc.jwt);
  const midVc = await issueCredential(
    LEAF_DID, {delegatedFrom}, MID_DID, midKp, 3600
  );
...
describe('extractDelegatedFrom', () => {
  it('returns null when no delegatedFrom in payload', async () => {
    ...
    const {parseCredential} = await import('../core/vc.js');
    const payload = parseCredential(vc.jwt);
    expect(payload).not.toBeNull();
    expect(extractDelegatedFrom(
      /** @type {NonNullable<typeof payload>} */ (payload)
    )).toBeNull();
  });
  ...
  it('returns the delegatedFrom value', async () => {
    ...
    const vc = await issueCredential(
      LEAF_DID, {delegatedFrom: 'abc123'}, ROOT_DID, kp
    );
    const {parseCredential} = await import('../core/vc.js');
    const payload = parseCredential(vc.jwt);
    expect(payload).not.toBeNull();
...
describe('verifyDelegationChain', () => {
  ...
  it('succeeds for a 3-hop chain', async () => {
    ...
    const midVc = await issueCredential(
      MID2_DID, {delegatedFrom: delegatedFrom1}, MID_DID, midKp, 3600
    );
    const delegatedFrom2 = sha256base64url(midVc.jwt);
    const mid2Vc = await issueCredential(
      LEAF_DID, {delegatedFrom: delegatedFrom2}, MID2_DID, mid2Kp, 3600
    );
  ...
  it('fails when issuer doesn\'t match prior subject', async () => {
    ...
    const rootVc = await issueCredential(MID_DID, {}, ROOT_DID, rootKp, 3600);
    // This VC is issued by WRONG_ISSUER_DID (should be issued by MID_DID
    // to continue the chain)
    const wrongVc = await issueCredential(
      LEAF_DID, {}, WRONG_ISSUER_DID, wrongIssuerKp, 3600
    );

    const result = await verifyDelegationChain(
      [
        {vcJwt: rootVc.jwt, issuerPublicKey: rootKp.publicKey},
        {vcJwt: wrongVc.jwt, issuerPublicKey: wrongIssuerKp.publicKey}
      ],
      LEAF_DID
    );
  ...
  it('fails on invalid signature in chain', async () => {
    const {midKp, rootVc, midVc} = await make2HopChain();
    const wrongKp = await generateKeyPair();
    const result = await verifyDelegationChain(
      [
        {vcJwt: rootVc.jwt, issuerPublicKey: wrongKp.publicKey},
        {vcJwt: midVc.jwt, issuerPublicKey: midKp.publicKey}
      ],
      LEAF_DID
    );
  ...
  it('fails when expired link in chain', async () => {
    ...
    // expired
    const rootVc = await issueCredential(MID_DID, {}, ROOT_DID, rootKp, -1);
    const delegatedFrom = sha256base64url(rootVc.jwt);
    const midVc = await issueCredential(
      LEAF_DID, {delegatedFrom}, MID_DID, midKp, 3600
    );
  ...
  it('detects circular reference via seen-issuer set', async () => {
    ...
    const result = await verifyDelegationChain(
      [
        {vcJwt: vc1.jwt, issuerPublicKey: rootKp.publicKey},
        {vcJwt: vc2.jwt, issuerPublicKey: rootKp.publicKey}
      ],
      LEAF_DID
    );
